...
class AutoTags:
    ...
    def clean_document(self, document):
        ...
        document = ' '.join(
            [i for i in document.split() if i not in STOPWORDS])
        return document
    ...
    def extract_subject(self, document):
        ...
        top_10_entities = [w for w, c in nltk.FreqDist(
    ...
    def get_auto_tags_from_document(self, text, doc_id):
        ...
        if len(entities) == 0:
            ...
            auto_tags = [
                word for word in auto_tags if word not in STOPWORDS and len(word) > 2]
            return auto_tags, []
        ...
        for svo in svos:
            if svo:
                svo_word = svo["subject"] + " " + \
                    svo["action"] + " " + svo["object"]
